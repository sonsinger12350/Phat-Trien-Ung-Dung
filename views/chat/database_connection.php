...
$connect = 
new PDO("mysql:host=localhost;dbname=id16893036_dulieu","id16893036_son","Aptx4869+4869");
...
function fetch_user_chat_history($from_user_id, $to_user_id, $connect)
{
	...
	foreach($result as $row)
	{
		...
		if($row["from_user_id"] == $from_user_id)
		{
			if($row["status"] == '2')
			{
				$chat_message = '<em>Tin Nhắn Này Đã Bị Thu Hồi</em>';
...
{
	$query = "SELECT ten FROM users WHERE user_id = '$user_id'";
	$statement = $connect->prepare($query);
	$statement->execute();
	$result = $statement->fetchAll();
	foreach($result as $row)
...
function fetch_group_chat_history($connect)
{
	...
	$query = "
	SELECT * FROM chat_message 
	WHERE to_user_id = '0' and lophoc = '$lophoc'
	ORDER BY timestamp ASC
	";
